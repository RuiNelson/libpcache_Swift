...
///
/// `Data().withUnsafeBytes`, an empty `RawSpan`, and similar containers may yield a buffer whose `baseAddress` is
/// `nil`. Forwarding such a `nil` to C is undefined behaviour even when the count is zero, so we substitute a sentinel
/// address. The C layer never reads from it because every code path that uses this helper either validates the size
/// first (rejecting empties of the wrong shape) or early-returns when the count is zero.
